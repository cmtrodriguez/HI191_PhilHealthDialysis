...
export default function DoctorsDirectoryView({
  ...
  // Dynamic Accreditation & Profile Warning Modal States
  const [confirmModal, setConfirmModal] = useState<{
    isOpen: boolean;
    type: 'grant' | 'revoke' | 'delete';
    targetDoctor: Nephrologist | null;
  }>({ isOpen: false, type: 'grant', targetDoctor: null });
  ...
  const handleToggleStatusClick = (doc: Nephrologist) => {
    // If it's currently active, clicking means we want to REVOKE. Otherwise, GRANT.
    const targetType = doc.isActive ? 'revoke' : 'grant';
    setConfirmModal({ isOpen: true, type: targetType, targetDoctor: doc });
  };

  const handleDeleteClick = (doc: Nephrologist) => {
    setConfirmModal({ isOpen: true, type: 'delete', targetDoctor: doc });
  };

  const handleExecuteAction = () => {
    const { type, targetDoctor } = confirmModal;
    if (!targetDoctor) return;

    if (type === 'delete') {
      onDeleteDoctor(targetDoctor.id);
    } else {
      // Toggle the structural isActive boolean mapping
      onUpdateDoctor({
        ...targetDoctor,
        isActive: type === 'grant',
      });
    ...
    // Clear operational scopes
    setConfirmModal({ isOpen: false, type: 'grant', targetDoctor: null });
  };
  ...
  return (
    <div className="space-y-6">
            ...
            >
              ...
              <td className="px-6 py-4 text-right">
                <div className="flex items-center justify-end gap-2">
                  ...
                  {/* REPLACE THE OLD ACCREDITATION BUTTON AND KEEP THE ACCREDITATION LABELS */}
                  <button
                    onClick={() => handleToggleStatusClick(doc)}
                    className={`p-2 px-3 text-xs font-bold rounded-xl border transition-all active:scale-95 flex items-center gap-1.5 ${
                      doc.isActive 
                        ? 'bg-emerald-50 text-emerald-600 border-emerald-100 hover:bg-emerald-100/70' 
                        : 'bg-slate-50 text-slate-400 border-slate-200 hover:bg-slate-100'
                    }`}
                    title={doc.isActive ? "Revoke Accreditation Status" : "Grant Accreditation Status"}
                  >
                    {doc.isActive ? (
                      <>
                        <ToggleRight size={16} />
                      ...
                      <>
                        <ToggleLeft size={16} />
                        <span>Grant Accreditation</span>
                      </>
                    )}
                  </button>

                  {/* Leave your trash/purge button right below it unchanged */}
                  <button
                    onClick={() => handleDeleteClick(doc)}
                    className="p-2 hover:bg-rose-50 text-slate-400 hover:text-rose-600 border border-transparent hover:border-rose-100 rounded-xl transition-all active:scale-95"
      ...
      {/* SECURITY OVERLAY AND CONFIRMATION DIALOG MATRIX */}
      <AnimatePresence>
        {confirmModal.isOpen && confirmModal.targetDoctor && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
            ...
            >
              {/* Dynamic Header UI Assignment based on Action Severity */}
              {confirmModal.type === 'grant' && (
                <>
                  ...
                  <div>
                    <h3 className="text-base font-bold text-slate-800 tracking-tight">Grant Accreditation</h3>
                    <p className="text-slate-400 text-xs mt-1.5 leading-relaxed">
                      Are you sure you want to activate and authorize **Dr. {confirmModal.targetDoctor.first} {confirmModal.targetDoctor.last}**? This enables their cryptographic signature mapping on digital PhilHealth claims documents.
                    </p>
                  </div>
              ...
              {confirmModal.type === 'revoke' && (
                <>
                  ...
                  <div>
                    <h3 className="text-base font-bold text-slate-800 tracking-tight text-amber-900">Revoke Accreditation</h3>
                    <p className="text-slate-400 text-xs mt-1.5 leading-relaxed">
                      Warning: Revoking **Dr. {confirmModal.targetDoctor.last}** will suspend their medical profile context. They will be blocked from signing pending dialysis approvals or session claim sheets until reinstated.
                    </p>
                  </div>
                  <div className="flex gap-2.5 pt-2">
                    <button
                    ...
                    <button
                      type="button"
                      onClick={handleExecuteAction}
                      className="flex-1 h-11 rounded-xl text-xs font-bold text-white bg-amber-600 hover:bg-amber-700 shadow-md shadow-amber-100 transition-colors"
                    >
                      Confirm Revocation
                    </button>
                  </div>
              ...
              {confirmModal.type === 'delete' && (
                <>
                  ...
                  <div>
                    <h3 className="text-base font-bold text-slate-800 tracking-tight">Purge Physician Records</h3>
                    <p className="text-slate-400 text-xs mt-1.5 leading-relaxed">
                      Are you absolutely sure you want to wipe <b>Dr. {confirmModal.targetDoctor.first} {confirmModal.targetDoctor.last}</b> from your internal repository files completely? This parameters structure index can't be undone.
                    </p>
                  </div>
                  <div className="flex gap-2.5 pt-2">
                    <button
